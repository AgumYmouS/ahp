...
class ResultController extends Controller
{
    ...
    public function storeFuzzyKri(){
        ...
        if ($dataKriteria != null) {
          ...
          foreach ($dataKriteria as $key => $value) {
            ...
            $data = ["sintesis" => $sintesis, "vektor"=>$vektoArray];
    ...
    public function storePemasok(){
      ...
      $pemasoks = Pemasok::all();
      $experts    = Expert::all();
      $pemasokSubs = PemasokSub::orderBy('expert_id','ASC')->orderBy('kriteria_id','ASC')->orderBy('pemasok1_id','ASC')->get();
          ...
          $kkr = $kk->where('kriteria_id','=',$kvalue->id);
              ...
              foreach ($pemasoks as $k1) {
                ...
                foreach ($pemasoks as $k2) {
                  $found1 = $kkr->where('pemasok1_id', '=', $k1->id)->where('pemasok2_id', '=', $k2->id)->first();
                  $found2 = $kkr->where('pemasok1_id', '=', $k2->id)->where('pemasok2_id', '=', $k1->id)->first();

                  if($found1 != null){
                    $dataKriteria[$e->id][$kvalue->id][$k1->id][] = $found1['nilai'];
                  }elseif($found1 == null && $found2 != null){
                    $dataKriteria[$e->id][$kvalue->id][$k1->id][] = 1/$found2['nilai'];
                  }else{
                    $dataKriteria[$e->id][$kvalue->id][$k1->id][] = 1;
                  }
