...
class PemasokSubController extends AppBaseController
{
    ...
    public function create($id)
    {
      ...
      $ps = PemasokSub::where('expert_id','=',$id)->orderBy('kriteria_id','ASC')->orderBy('pemasok1_id','ASC')->get();
      ...
      foreach ($kriterias as $kk => $vk) {
        ...
        if($ck < countTable($pemasoks->count()) * $hit){
          $selectedKri = $vk;
          break;
        }
      }
        if ($selectedKri != null) {
            $hms = $kriterias->where('id', '=', $selectedKri->id);
  
            foreach ($hms as $key => $value) {
                $cSub = PemasokSub::all()->where('kriteria_id', '=', $value->id)->where('expert_id', '=', $id)->count();
            ...
            $csub = Kriteria::all()->count();
            $pemasok2 = null;
            $ketemu = false;
            // dd(countTable($pemasoks->count())*$csub, $pemasokSubs->count());
            $full = countTable($pemasoks->count())*$csub == $pemasokSubs->count() ? true : false;
            foreach ($pemasoks as $p1) {
                ...
                $count = $pemasokSubs->where('pemasok1_id', '=', $p1->id)->where('kriteria_id', '=', $selectedKri->id)->count() +
                  $pemasokSubs->where('pemasok2_id', '=', $p1->id)->where('kriteria_id', '=', $selectedKri->id)->count();
                $countKri = $kriterias->where('id', '=', $selectedKri->id);
                ...
                foreach ($pemasoks as $p2) {
                    if ($p1->id != $p2->id) {
                        ...
                        $ada1 = $pemasokSubs->where('pemasok1_id', '=', "$p1->id")->where('pemasok2_id', '=', "$p2->id")->where('kriteria_id', '=', $selectedSub->id)->count();
                        $ada2 = $pemasokSubs->where('pemasok1_id', '=', "$p2->id")->where('pemasok2_id', '=', "$p1->id")->where('kriteria_id', '=', $selectedSub->id)->count();
                        if ($ada1 == 0 && $ada2 == 0) {
                            $pemasok1 = $p1;
                            $pemasok2 = $p2;
                            $ketemu = true;
      ...
      return view('pemasok_subs.create')->with('expert', $expert)
                  ...
                  ->with('kriteriases', $kriterias)->with('pemasoks', $pemasoks)
                  ->with('kriterias', $selectedSub)->with('selectedSub', $selectedSub)
    ...
    public function edit($id)
    {
        ...
        $pemasok1 = Pemasok::find($pemasokSub->pemasok1_id);
        $pemasok2 = Pemasok::find($pemasokSub->pemasok2_id);
        $kriteria = Kriteria::find($pemasokSub->kriteria_id);
